...
public class PageAdminVaccineHub extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == back){
            ...
            String a = String.valueOf(JOptionPane.showInputDialog("Vaccine's name : "));
            Vaccine found = DataIO.checkVaccine(a);
            if(found != null) {
                DefaultTableModel tempModel = (DefaultTableModel) table.getModel();
                tempModel.setRowCount(0);
                for(Vaccine c : DataIO.allVaccine){
                    if(c.equals(found)){
                        tempModel.addRow(new Object[]{c.getName(),c.getDayRequired(),c.getDoseRequired(),c.getAmount()});
                    }
                }
            }
            else {
                JOptionPane.showMessageDialog(search, "No Vaccine Found!");
            }
        }
        else if(e.getSource() == register){
            setVisible(false);
            dispose();
            PageAdminVaccineRegister adminVaccineRegister = new PageAdminVaccineRegister();
            adminVaccineRegister.setVisible(true);
        }
    }
...
//        mainPanel.add(t);
        ...
        for(int i = 0; i<size; i++){
            ...
            data[i][0] = a.getName();
            data[i][1] = String.valueOf(a.getDayRequired());
            data[i][2] = String.valueOf(a.getDoseRequired());
            data[i][3] = String.valueOf(a.getAmount());
        }
        String[] column = {"Vaccine", "Day Required", "Dose Required", "Amount in Stock"};
        ...
        table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                if(!e.getValueIsAdjusting()){ //to only invoked the selection only once
                    ...
                    if(x.equals("-")){
                        JOptionPane.showMessageDialog(null,"Data Cannot be Viewed/Modified!");
                    }
                    else{
                        Main.currentVaccine = DataIO.checkVaccine(x);
                        setVisible(false);
                        dispose();
                        PageAdminVaccineView vaccineSupplyView = new PageAdminVaccineView();
                        vaccineSupplyView.setVisible(true);
                    }
                }
            }
        });
